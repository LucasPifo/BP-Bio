foreach(DBForm('Programación de Servicios')->WHERE('Estatus Programación','=','Programando')->get() AS $Programacion){
    ...
    $semanas_2veces = [];
    $Meses = [];
    foreach(DBForm('Equipos con Sensibilidad')->WHERE('Idprogr','=',$Programacion['ID'])->get() AS $Equiposensible){
        array_push($areas, $Equiposensible['Equipo Sensible']);
        array_push($frecuancias, $Equiposensible['Frecuencia de Inspeccion']);
        array_push($dias, $Equiposensible['Dia']); 
        array_push($semanas, $Equiposensible['Numero de Semana']); 
        array_push($semanas_2veces, $Equiposensible['Semana']);
        array_push($Meses, $Equiposensible['Cada Cuantos Meses']);
    }
    $fechasTotales = [];
    $areasFecha = [];
    $frecuanciastotales = [];
    $cantareas=count($areas);
    for($c=0; $c<$cantareas; $c++){
    ...
    for($i=0;$i<$no_fech_no_rep;$i++){
        ...
        $visita['Areas con Equipos sensibles para esta fecha']=implode(',',$areas_por_fecha);
        save($visita);
