foreach(DBForm('Programación de Servicios')->WHERE('Estatus Programación','=','Programando')->get() AS $Programacion){
    ...
    $semanas_2veces = [];
    $Meses = [];
    foreach(DBForm('Materia Prima con Sensibilidad')->WHERE('Idprogr','=',$Programacion['ID'])->get() AS $Equiposensible){
        array_push($areas, $Equiposensible['Materia Prima Sensible']);
    ...
    for($i=0;$i<$no_fech_no_rep;$i++){
        ...
        $visita['Materias Primas para esta fecha']=implode(',',$areas_por_fecha);
        save($visita);
