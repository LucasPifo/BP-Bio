foreach(DBForm('Administración de Servicios')->WHERE('Bandera','=','1')->WHERE('Estatus','=','Generando')->get() AS $stockRow){
    $Contador=0;
    foreach(DBForm('Nivel de Escalación')->WHERE('Idcon','=',$stockRow['idcont'])->WHERE('Tipo de servicio','=','Inspeccion de Feromonas')->get() AS $Nivel){
        $Contador++;
    }
    if($stockRow['tipo_programacion']=="Normal"){
        foreach(DBForm('Areas del Servicio')->WHERE('IDPROG','=',$stockRow['ID Programación'])->get() AS $AreasDisp){
            foreach(DBForm('Asignacion de Dispositivos')->WHERE('idCON','=',$stockRow['idcont'])->WHERE('IDAREA','=',$AreasDisp['IdArea'])->WHERE('Clasificacion Dispositivo','=','FE')->get() AS $Asignacion){
                ...
                if($Contador > 0){
                    ...
                    foreach(DBForm('Nivel de Escalación')->WHERE('Idcon','=',$stockRow['idcont'])->WHERE('Tipo de servicio','=','Inspeccion de Feromonas')->get() AS $Nivel2){
        ...
        foreach(DBForm('Areas del Servicio')->WHERE('IDPROG','=',$stockRow['ID Programación'])->get() AS $Areasescala){
            ...
            $discant=count($dispo);
            foreach(DBForm('Asignacion de Dispositivos')->WHERE('idCON','=',$stockRow['idcont'])->WHERE('Area','=',$Areasescala['Areas con Escalacion'])->WHERE('Clasificacion Dispositivo','=','FE')->get() AS $Adis){
                for($i=0;$i<$discant;$i++){
                    if($dispo[$i]==$Adis['Numero de dispositivo QR']){                 
                        $Dispositivo=NewRegister('Feromonas');
                        $Dispositivo['No.Dispositivo']=$Adis['Numero de dispositivo QR'];
                        $Dispositivo['Contrato']=$stockRow['Contrato'];
                        $Dispositivo['Llave QR']=$stockRow['N.S.T'].$Asignacion['Numero de dispositivo QR'];
                        $Dispositivo['IDASIGD']=$Adis['Consecutivo'];
                        $Dispositivo['Area']=$AreasDisp['Area'];
                        $Dispositivo['N.S.T']=$stockRow['N.S.T'];
                        $Dispositivo['Cliente']=$stockRow['Cliente'];
                        $Dispositivo['Fecha']=$stockRow['Fecha de ejecucion'];
                        $Dispositivo['Tecnico que firma']=$AreasDisp['Tecnico'];
                        ...
                        if($Contador > 0){
                            ...
                            foreach(DBForm('Nivel de Escalación')->WHERE('Idcon','=',$stockRow['idcont'])->WHERE('Tipo de servicio','=','Inspeccion de Feromonas')->get() AS $Nivel3){
                      ...
                      $res=explode( '-', $numerito );	
                      $resultado=array_pop($res);
                      $Dispositivo['Numerodispositivoabre']="FE-".$resultado;
                      save($Dispositivo);
                    }
                }
            }
